...
export class ShopDataSource {
    ...
    static getProduct(shopId: string, company: string, product_brand?: string): Promise<ProductEntity[]> {
        return httpClient.get(`${BASE_URL_SOHEE}/v1/sellcoda/shops/${shopId}/products/prices?limit=100&company=${company}`)
            .then(res => res.data.products)
            .catch(error => console.log(error))
    }

    static getProductWithBrand(shopId: string, company: string, product_brand?: string): Promise<ProductEntity[]> {
        return httpClient.get(`${BASE_URL_SOHEE}/v1/sellcoda/shops/${shopId}/products/prices?limit=100&company=${company}&product_brand=${product_brand}`)
            .then(res => res.data.products)
            .catch(error => console.log(error))
    }
    static getShopById(shopId: string): Promise<ShopEntity> {
        return httpClient.get(`${BASE_URL_SOHEE}/v1/sellcoda/customers/dealers/${shopId}`)
            .then(res => res.data)
            .catch(error => console.log(error))
    }

    static getBrandByShopId(shopId: string, company: string): Promise<BrandEntity[]> {
        return httpClient.get(`${BASE_URL_SOHEE}/v1/sellcoda/customers/dealers/${shopId}/product_brand?company=${company}`)
